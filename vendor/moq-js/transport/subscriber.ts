...
export class Subscriber {
  ...
  async recvPublishNamespace(msg: Control.PublishNamespace) {
    if (this.#publishedNamespaces.has(msg.namespace.join("/"))) {
      throw new Error(
        `duplicate publish namespace for namespace: ${msg.namespace.join("/")}`,
      );
    }
    ...
    const publishNamespace = new PublishNamespaceRecv(
      this.#control,
      msg.namespace,
      msg.id,
    );
    ...
    this.#publishedNamespacesQueue.update((queue) => [
      ...queue,
      publishNamespace,
    ]);
  }
  ...
  async unsubscribe(track: string) {
    if (this.#trackToIDMap.has(track)) {
      ...
      console.warn(
        `During unsubscribe request initiation attempt track ${track} not found in trackToIDMap.`,
      );
    }
  }
  ...
  async recvObject(reader: TrackReader | SubgroupReader) {
    ...
    if (subscriptionId === undefined) {
      console.warn(
        `Exception track alias ${track_alias} not found in aliasToSubscriptionMap.`,
      );
